...
//inline NLNET::TServiceId GetSID( RPC_SESSION session )
//{
//    uint16 sid =  session & 0x7f;
//    return NLNET::TServiceId(sid);
//}

inline uint16 GetSID( RPC_SESSION session )
{
    return session & 0x7f;
}

inline double tofixed3( double num )
{
    sint64 si = (num+0.0005) * 1000;
    return (double)si/1000;
}
...
inline double calctrl(double BOTTOM, double TOP, double val)
{
    double ret = 0;
    ...
    {
        ret = 1-((val-BOTTOM)/(TOP-BOTTOM));
    }

    return ret;
}
inline double percent_ctrl(double BOTTOM, double TOP, double val)
{
    //const double SUB = (TOP-BOTTOM) + 0.0000005;
    //const double SCR = 100 / SUB;
    //double ret = 0;
    ...
    //else
    //{
    //    ret = (SUB - (val - BOTTOM)) * SCR;
    //}

    return tofixed3( calctrl(BOTTOM, TOP, val)*100 );
}

static NLMISC::CRandom _MainRandom;
struct CPMainRandom { CPMainRandom() { _MainRandom.srand( NLMISC::CTime::getLocalTime() ); } };
static CPMainRandom mainRandom = CPMainRandom();

inline sint32 GetRandom() { return _MainRandom.rand(); }                    /// generate a random value in [0, CRandom.RandMax]
inline sint32 GetRandom( sint16 mod ) { return _MainRandom.rand( mod ); }   /// generate a random value in [0, mod]
inline sint32 GetRandom( sint16 min_dig, sint16 max_dig )
{
    if ( min_dig>max_dig || min_dig<0  || max_dig<0 )
    {
        nlwarning( "min_dig is bigger than max_dig, min_fig: %d, max_dig: %d ", min_dig, max_dig );
        return -1;
    }
    sint16 dig = max_dig - min_dig;
    return _MainRandom.rand( dig ) + min_dig;
}

inline uint32 GetUint32Random(uint32 min_dig, uint32 max_dig)
{
    if ( min_dig>max_dig )
    {
        nlwarning( "min_dig is bigger than max_dig, min_fig: %d, max_dig: %d ", min_dig, max_dig );
