...
public:
    ...
    typedef  std::set<PID>     _TPIDs;
    typedef  std::vector<PID>  _TVctPIDs;
    ...
    typedef  std::map<PID, SCORE>     TMapScore;
    typedef  std::map<SCORE, _Ranking>     TMapRanking;
	...
	*	\param  nScore
	*/
	void SetScore( const PID pid, const SCORE score )
	{
        RANKING nRankingNew = 0;
	...
	*/
	RANKING GetRanking( const SCORE score )
	{
        RANKING  rnk = 0;
    ...
    SCORE GetScore( PID unPlayerID )
    {
        std::map<PID, SCORE> ::iterator it = _MapScore.find( unPlayerID );
        if ( it == _MapScore.end() ) {
            return 0;
        }
