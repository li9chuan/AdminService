...
bool launchProgram (const std::string &programName, const std::string &arguments, std::vector<char *>* argvi)
{
...
*/
	...
	else if (status == 0)
    {
        ...
        if (argvi!=NULL)
        {
            status = execvp(programName.c_str(), &argvi->front());
        }
        else
        {
	...
	{
		//nldebug("LAUNCH: Successful launch '%s' with arg '%s'", programName.c_str(), arguments.c_str());
        while(wait(&exitstatus)!=status);//¸¸½ø³ÌµÈ´ý
        nldebug("child exited with status %d,%d\n",exitstatus>>8,exitstatus&0377);
		return true;
	}
#else
