...
public:
	...
	*	\return 排名从1开始，0表示异常
	*/
	RANKING SetScore( const PID nID, const SCORE nScore )
	{
		RANKING nRankingNew = GetIDRanking(nID);

		if ( nRankingNew == 0 )
		{
			RANKING nRanking = _addRankingBack( nID, nScore );
	...
	*/
	RANKING GetRanking( const PID nID )
	{
	...
	uint32 GetPage( const PID nID, const uint32 nPageRow )
	{
		uint32 page = 0;
		RANKING ranking = GetRanking( nID );
	...
	/// 获得分页
	bool GetPage( const uint32 nPage, const uint32 nPageRow, std::vector<PID>& vct )
	{
		vct.clear();
...
private:
	...
	*	\return 调整后，玩家的排名
	*/
	RANKING _adjustRanking( const RANKING nRanking, const SCORE nScore )
	{
		RANKING nNewRanking = nRanking;
	...
	*	\return 加入后，玩家的排名
	*/
	RANKING _addRankingBack( const PID nID, const SCORE nScore )
	{
	...
	void _updateMapRanking( const RANKING nRanking )
	{
        SetIDRanking( _vctScore[nRanking].nID, nRanking );
	}

	RANKING _upRanking( const RANKING nOldRanking, const SCORE nScore )
	{
		RANKING nNewRanking = nOldRanking;
	...
	RANKING _downRanking( const RANKING nOldRanking, const SCORE nScore )
	{
		RANKING nNewRanking = nOldRanking;
		...
		/// 排名下移
		for ( nNewRanking; nNewRanking<RANKING(_vctScore.size()-1); ++nNewRanking )
		{
			if (_vctScore[nNewRanking+1].nScore>nScore )
			{
				_vctScore[nNewRanking] = _vctScore[nNewRanking+1];
...
private:
    ...
    typedef std::map<PID, RANKING>     TIDRanking;
    TIDRanking                              _HashRankingSecond;     ///  二级哈希空间

    ///  return hash idx
    inline uint HashFunctionPID( PID pid )
    {
        //return (pid>>10) & HashSpaceMask;
        return (pid) & HashSpaceMask;
    }

    RANKING SetIDRanking( PID pid, RANKING ranking )
    {
